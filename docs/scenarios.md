...
| Declaration mismatch | New registration dispatch names a declaration outside the selected release and current authorized upgrade candidate. | ReleaseMismatch before owner dispatch; original records and progress stay unchanged. |
| Superseded observation | Delayed success/failure mismatches the exact stored unresolved attempt, or names an attempt replaced by newer evidence. | StaleSnapshot; no progress overwrite, readiness credit or lifecycle move. Exact still-pending observation recovery is distinguished before treating a historical context as stale. |
...
| Forged candidate exception | Seek new dispatch/readiness credit with a future/failed/foreign candidate, omit candidate context, or mismatch configuration/bindings/generation. | Refuse before owner dispatch or readiness credit; selection and authority remain unchanged. A supplied historical context alone cannot invoke pending-attempt recovery. |
